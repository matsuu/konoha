#!/bin/sh
# $Id: provider-register.sh,v 1.1 2007/05/17 02:39:01 tyreld Exp $
# ==================================================================
...
sfcb_rebuild()
{
	...
	/usr/local/bin/sfcbrepos -f $endian
	if test $? != 0
	then
...
{    
	...
	/usr/local/bin/sfcbstage -n $namespace -r $_REGFILENAME $mymofs
	if test $? != 0 
	then
...
{    
    ...
    /usr/local/bin/sfcbunstage -n $namespace -r $baseregname.reg $mymofs
