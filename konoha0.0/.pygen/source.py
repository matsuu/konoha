...
class Source :
	def __init__(self, bdir):
		...
		self.XPACKAGE_LIST = []
		self.INCLUDE_H = ['<konoha/class/konoha.h>']
		self.STATICLIB_MAP = {}
...
def functype_filter(text):
	text = text.replace('PUBLIC', '').replace('PRIVATE', '').replace('INLINE','')
	prev = ' '
	text2 = ''
	for c in text:
		if c == ' ' and prev == ' ':
...
def read_source_cblock(fpath, zone, text, data):
	...
	if functype == None or functype.find('static') >= 0: return
	...
	if text.find('/* @map') != -1: data.add_mapper(parse_anno(text, '@map'), funcname)
	if text.find('/* @method') != -1: data.add_method(parse_anno(text, '@method'), functype)
...
def read_include_class(dir, data):
	...
	read_source_h(dir + '/konoha.h', data)
	fl = os.listdir(dir)
	for fn in fl:
		if fn.endswith('_.h'): continue
		if fn == 'konoha.h': continue
		if fn.endswith('.h'): 
			read_source_h(dir + '/' + fn, data)
			data.INCLUDE_H.append('<konoha/class/%s>' % fn)
