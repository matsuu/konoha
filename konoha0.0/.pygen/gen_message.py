...
''' % (func))
    ...
    func = 'void knh_Compiler_setLang(char *lang)'
    CPROTO.append(func+';')
    f.write('''
%s
{
...
static char* knh_message_data[] = {''')
    ...
    /* %s */ ''' % m.KMSG)
        if m.lmap.has_key("en"):
            f.write('''
        /* en */ "%s",
#ifdef KNH_USING_ICONV''' % (sjis_filter(m.lmap["en"])))
        else:
            f.write('''
        /* en */ NULL,
#ifdef KNH_USING_ICONV''')
...
def gen_message(bdir):
    fname = '%s/konoha/gen/konoha_message_.c' % bdir
